...
**Лучший результат:** `UNet + TTA + blend(a=0.8)` → **PSNR 23.91 дБ, SSIM 0.6129** на 368 val-картинках.
Это **+0.62 дБ PSNR** над лучшим классическим методом (bilateral+bicubic). Модель — `unet_v2.pth` после fine-tune с L1-loss.
...
После первого обучения дообучила модель с другими настройками, стартуя с `unet.pth`:

- **Loss:** L1 вместо MSE (меньше «мыла» на краях)
- **LR:** 1e-4 (стандарт для fine-tune)
- **Scheduler:** `ReduceLROnPlateau(patience=2, factor=0.5)` — из лекции 15 про оптимизацию
- **Save-best:** лучший чекпойнт пишется в `models/unet_v2.pth`
- **Эпох:** прерван на 5-й эпохе (CPU, плато по train_loss)

Результат: v2 побил v1 на +0.13 дБ PSNR и +0.0075 SSIM (см. Results).
...
**bicubic | bilateral+bicubic | UNet+TTA | UNet+TTA+blend(a=0.8) | оригинал HR**
...
Финальная таблица на всех 368 val-картинках, модель `unet_v2.pth` (полный вывод в `results.txt`):
...
| UNet (v2) | 23.85 | 0.6115 |
| UNet+TTA | 23.87 | 0.6126 |
| **UNet+TTA+blend, a=0.8** | **23.91** | **0.6129** |
| UNet+TTA+blend, a=0.8 + USM@0.5 | 23.89 | 0.6122 |

Сравнение v1 vs v2 (на том же val-сплите и той же evaluate.py):

| Модель | PSNR | SSIM | лучший α |
|---|---|---|---|
| v1 (MSE, 10 эпох) | 23.78 | 0.6054 | 0.7 |
| v2 (+ L1 fine-tune) | **23.91** | **0.6129** | 0.8 |

**Ключевые выводы:**

- UNet (v2) уверенно обходит все классические бейзлайны: **+0.56 дБ** над лучшим классическим (bilateral+bicubic), **+0.032 SSIM**.
- Fine-tune с L1 и LR=1e-4 дал **+0.13 дБ PSNR и +0.0075 SSIM** поверх v1 — скромно, но стабильно и воспроизводимо.
- TTA почти не даёт прироста (+0.02 дБ) — как и у v1. Модель уже устойчива к горизонтальному флипу, контент датасета симметричный.
- Blend с bilateral+bicubic даёт ещё **+0.04 дБ** поверх TTA. Интересный момент: оптимум α сдвинулся 0.7 → 0.8 — после fine-tune у сети стало меньше «мыла», её вклад в смесь можно увеличить.
- **USM по-прежнему сделал хуже** при любом `amount > 0` — вывод из v1 подтвердился на v2. Гипотеза — добавить резкости поверх «мыла». По факту: UNet+blend уже убрал высокочастотные шумы, а USM именно высокие частоты усиливает, включая артефакты, отсутствующие в GT. Это прямо ложится на комментарий лектора к HW3: *«USM усиливает не только границы но и шум»*.

**Сильные стороны:**
- Воспроизводимые цифры (все RNG зафиксированы)
- Честная оценка — USM включён в таблицу даже тем что ухудшает результат
- Два шага обучения (MSE → L1 fine-tune) с честным сравнением v1 vs v2
- Три inference-time доработки грамотно вытягивают +0.06 дБ без переобучения
- Bigger-than-baseline прирост и на PSNR, и на SSIM

**Слабые стороны:**
- Визуально всё ещё немного «мыло» — L1 уменьшил, но не убрал (прирост +0.13 дБ — это шаг в верную сторону, но не скачок)
- Модель училась на CPU, fine-tune прервала на 5-й эпохе из-за плато по train_loss
...
- Fine-tune с L1 и LR=1e-4 добавил +0.13 дБ поверх v1 — подтвердил гипотезу про «мыло» MSE
...
**Будущая работа:**
- **Дообучить на GPU до сходимости** — на CPU прервала на 5-й эпохе fine-tune, train_loss вышел на плато, но на GPU можно было бы спокойно добить 20–30 эпох и посмотреть, сдвинется ли
...
- [x] Fine-tune с L1 + ReduceLROnPlateau (`finetune.py`, v2-чекпойнт)
- [x] Повторная оценка v2 на 368 val-картинках
